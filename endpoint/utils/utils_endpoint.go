...
func WriteHttpResponseResult(
	token bool,
	w http.ResponseWriter,
	r *http.Request,
	handler func(interface{}, *dto.UserContext) (dto.Response, error),
) {
	var (
	...
	// Handle JSON or Form Data requests (non-multipart)
	...
	handleAndRespond(w, r, handler, requestBody, contextModel)
}
...
func extractParam(
	...
	for key, value := range vars {
		if key == "ID" { //specifically desining pathParam key word ID must an integer
...
func handleAndRespond(
	...
	response, err := handler(requestData, contextModel)
	if err != nil {
